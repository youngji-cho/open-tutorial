function dashboard(id, fData){
  ...
  function histoGram(fD){
    ...
    let hGsvg = d3.select(id).append("svg")
      .attr("width", hGDim.w+hGDim.l+hGDim.r)
      .attr("height",hGDim.h+hGDim.t+hGDim.b)
      .attr("transform", "translate(" + hGDim.l + "," + hGDim.t + ")");

    //create function for x-axis mapping
    let x = d3.scaleLinear.ordinal().rangeRoundBands([0, hGDim.w], 0.1)
            .domain(fD.map(function(d) { return d[0]; }));

    // Add x-axis to the historgram svg.
  ...
  let tF=['low','mid','high'].map(function(d){
    return {type:d,freq:d3.sum(fData.map(function(t){t.freq[d]}))}
  });

  //caculate total frequency by state for all segement.
  let sF=fData.map(function(d){
    return [d.State,d.total];
  });

  let hG=histoGram(sF),//create historgram.
      pC=pieChart(tF), //create pie-chart.
